...
- [x] ISC-REL-6: the check runs only after a command and never for `self-update`; any failure in the check (no HOME, cache error, network/API error) leaves the command's exit status unchanged | Verify: Read: `main` invokes the check at the end for every command except `SelfUpdate`, and the check returns without surfacing errors
